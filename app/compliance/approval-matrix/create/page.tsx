...
import {
  ArrowLeft, ShieldCheck, ChevronDown, AlertCircle, Loader2, Save,
} from "lucide-react";
...
export default function CreateComplianceApprovalMatrixPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    Promise.all([
      fetch("/api/compliance/approval-matrix", CREDS).then((r) => r.ok ? r.json() : []),
  ...
  useEffect(() => {
    ...
    const required = rec?.template?.approval_levels ?? 1;
  ...
  async function handleSave() {
    setError("");
    if (!selectedId)  { setError("Please select a compliance record."); return; }
    ...
    setSaving(true);
    try {
      const res  = await fetch(`/api/compliance/${selectedId}/approval-matrix`, {
        method: "POST", headers: { "Content-Type": "application/json" }, ...CREDS,
        body: JSON.stringify({ levels }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Failed to save."); return; }
      router.push("/compliance/approval-matrix");
    } catch {
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Configure approvers for a compliance record</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">
            Compliance Record<span className="text-red-500 ml-0.5">*</span>
          </label>
          {loading ? (
            ...
            <div className="relative">
              ...
              >
                <option value="">— Select a compliance record —</option>
                {records.map((r) => (
                  <option key={r.id} value={r.id}>
                    {r.name || r.title || "Untitled"}{r.template ? ` — ${r.template.title}` : ""}
                  </option>
                ))}
              </select>
          ...
          {selectedRec?.template && (
            <p className="text-xs text-slate-400 mt-1.5">
              Template requires <strong>{selectedRec.template.approval_levels}</strong> approval level{selectedRec.template.approval_levels !== 1 ? "s" : ""}
            </p>
          )}
        </div>
        ...
        {!selectedId && !loading && (
          <div className="flex flex-col items-center py-8 text-slate-400">
            ...
            <p className="text-sm">Select a compliance record above to assign approvers.</p>
          </div>
        )}
