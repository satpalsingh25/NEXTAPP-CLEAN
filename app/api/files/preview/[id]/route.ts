...
/*                                                                      */
/* Serves a Document record (Compliance/AMC) inline for in-browser      */
/* rendering (PDF viewer, image display). Never exposes raw SharePoint  */
/* URLs or internal paths to the client.                                */
/* ------------------------------------------------------------------ */
export async function GET(
  ...
  /* Rate limit — 100 previews / 15 min per user */
  const rlCheck = checkRateLimit(user_id, "files-preview", "files");
  ...
  /* 7. Return inline (browser renders PDF/image in-place) ----------- */
  const contentType =
    spRes.headers.get("Content-Type")?.split(";")[0].trim() || mimeFromName(filename);
