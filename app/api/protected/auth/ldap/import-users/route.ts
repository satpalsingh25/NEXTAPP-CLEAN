/**
 * POST /api/protected/auth/ldap/import-users
 * Imports users from an LDAP directory into the application.
...
import { syncLdapUsers }                    from "@/lib/auth-providers/ldap/sync";
import { prisma }                           from "@/lib/prisma";

export async function POST(req: NextRequest): Promise<NextResponse> {
  const requestId = generateRequestId();
  const ctx       = { route: "POST /api/protected/auth/ldap/import-users", requestId };
  ...
  try {
    /* Load provider — SUPER_ADMIN can operate across companies */
    ...
    const result = await syncLdapUsers(provider.id, provider.company_id, auth.user.user_id);
    ...
    return errorResponse("User import failed. Please try again.", 500, requestId);
  }
}
