...
import { syncAzureGroups }     from "@/lib/auth-providers/azure/sync";

/**
 * POST /api/protected/auth/azure/sync-groups
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await syncAzureGroups(providerId, auth.user.company_id, auth.user.user_id);

    return successResponse(result, 200);
  } catch (e) {
    logInternalError(e, {
      route:      "POST /api/protected/auth/azure/sync-groups",
