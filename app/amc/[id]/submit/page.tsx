...
import { ArrowLeft, Send, Wrench, Calendar, Tag, AlertCircle } from "lucide-react";
...
export default function AMCSubmitPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/amc/${id}/submit`, {
        method: "POST",
        credentials: "include",
  ...
  if (!record) {
    return (
      <div className="max-w-xl mx-auto py-16 text-center">
        ...
        <Link href="/amc/my" className="mt-4 inline-block text-blue-600 hover:underline text-sm">
          Back to My AMC
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Submit AMC</h1>
        ...
        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <span className="font-medium text-slate-800">
              {record.amcTemplate?.approval_levels ?? "—"}
            </span>
          </div>
        </div>
      </div>
