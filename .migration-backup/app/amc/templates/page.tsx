...
export default function AMCTemplatesPage() {
  ...
  const resetForm = () => { setName(""); setFrequency("Yearly"); setApprovalLevels(1); setError(""); };

  const submit = async () => {
    if (!name.trim()) { setError("Name is required"); return; }
    setSaving(true);
    setError("");
    const res = await fetch("/api/amc/templates", {
      method: "POST",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">AMC Templates</h1>
          <p className="text-sm text-slate-500 mt-1">
            {loading ? "Loading…" : `${templates.length} template${templates.length !== 1 ? "s" : ""}`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={load} className="p-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors" title="Refresh">
            <RefreshCw size={16} />
          </button>
          <button
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-5">Create AMC Template</h2>
          {error && (
            <div className="mb-4 px-4 py-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg">{error}</div>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-3">
              ...
              <input
                ...
                placeholder="e.g. Annual Equipment Servicing"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl border border-slate-200 shadow-sm text-slate-400">
          ...
          <p className="font-medium">No AMC templates yet</p>
          <p className="text-sm mt-1">Create your first AMC template to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map((t) => (
            <div key={t.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-shadow">
              ...
              <h3 className="font-semibold text-slate-900 text-sm leading-snug mb-3">{t.name}</h3>
